...
Subject: cover thumbnail: the original compact gray dog SVG object with unchanged silhouette, angled closed eyes, black muzzle, rounded rectangular body, and small top-left handle/ear rendered as Dither card system
Source URL: manual reference folder
Stop rule: stop after 1 iterations or when a human approves a candidate.

## Inspect First

1. Open `outputs/runs/seed-cover-dither-card-system/contact-sheet.jpg`.
2. Read `outputs/runs/seed-cover-dither-card-system/source-review.json`.
...
5. Read `outputs/runs/seed-cover-dither-card-system/prompt.txt`.
6. Generate candidate assets using the prompt.
7. Save generated chroma-key images in `outputs/runs/seed-cover-dither-card-system/generated`.
...
python3 scripts/chroma_to_alpha.py \
  --input outputs/runs/seed-cover-dither-card-system/generated/<candidate>.png \
  --output outputs/runs/seed-cover-dither-card-system/cutouts/<candidate>-alpha.png \
  --key ff00ff
```
...
python3 scripts/build_contact_sheet.py \
  --input-dir outputs/runs/seed-cover-dither-card-system/cutouts \
  --output outputs/runs/seed-cover-dither-card-system/comparison.jpg \
  --columns 4 \
  --labels
```

Then update `outputs/runs/seed-cover-dither-card-system/taste-notes.md` with:
