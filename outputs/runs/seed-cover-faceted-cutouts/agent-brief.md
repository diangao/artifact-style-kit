...
Subject: cover thumbnail: the original compact gray dog SVG object with unchanged silhouette, angled closed eyes, black muzzle, rounded rectangular body, and small top-left handle/ear rendered as Faceted object cutouts
Source URL: manual reference folder
Stop rule: stop after 1 iterations or when a human approves a candidate.

## Inspect First

1. Open `outputs/runs/seed-cover-faceted-cutouts/contact-sheet.jpg`.
2. Read `outputs/runs/seed-cover-faceted-cutouts/source-review.json`.
...
5. Read `outputs/runs/seed-cover-faceted-cutouts/prompt.txt`.
6. Generate candidate assets using the prompt.
7. Save generated chroma-key images in `outputs/runs/seed-cover-faceted-cutouts/generated`.
...
python3 scripts/chroma_to_alpha.py \
  --input outputs/runs/seed-cover-faceted-cutouts/generated/<candidate>.png \
  --output outputs/runs/seed-cover-faceted-cutouts/cutouts/<candidate>-alpha.png \
  --key ff00ff
```
...
python3 scripts/build_contact_sheet.py \
  --input-dir outputs/runs/seed-cover-faceted-cutouts/cutouts \
  --output outputs/runs/seed-cover-faceted-cutouts/comparison.jpg \
  --columns 4 \
  --labels
```

Then update `outputs/runs/seed-cover-faceted-cutouts/taste-notes.md` with:
