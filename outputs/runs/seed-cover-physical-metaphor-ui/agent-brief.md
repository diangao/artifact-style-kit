...
Subject: cover thumbnail: one ripe mango with a small green leaf rendered as Physical-metaphor UI
Source URL: manual reference folder
Stop rule: stop after 1 iterations or when a human approves a candidate.

## Inspect First

1. Open `outputs/runs/seed-cover-physical-metaphor-ui/contact-sheet.jpg`.
2. Read `outputs/runs/seed-cover-physical-metaphor-ui/source-review.json`.
...
5. Read `outputs/runs/seed-cover-physical-metaphor-ui/prompt.txt`.
6. Generate candidate assets using the prompt.
7. Save generated chroma-key images in `outputs/runs/seed-cover-physical-metaphor-ui/generated`.
...
python3 scripts/chroma_to_alpha.py \
  --input outputs/runs/seed-cover-physical-metaphor-ui/generated/<candidate>.png \
  --output outputs/runs/seed-cover-physical-metaphor-ui/cutouts/<candidate>-alpha.png \
  --key ff00ff
```
...
python3 scripts/build_contact_sheet.py \
  --input-dir outputs/runs/seed-cover-physical-metaphor-ui/cutouts \
  --output outputs/runs/seed-cover-physical-metaphor-ui/comparison.jpg \
  --columns 4 \
  --labels
```

Then update `outputs/runs/seed-cover-physical-metaphor-ui/taste-notes.md` with:
